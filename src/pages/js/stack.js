...
export default class Stack {
  ...
  getBest() {
    ...
    return localStorage
      ? JSON.parse(localStorage.getItem(LS_HIGHEST_SCORE)) || defaultBest
      : defaultBest;
  }
  ...
  setBest(best) {
    this.best = best;
    localStorage &&
      localStorage.setItem(LS_HIGHEST_SCORE, JSON.stringify(best));
  }
  ...
  update(cube, apm) {
      ...
      lines.map((v, offsetX) => {
        if (!this.stack[y1 + offsetY]) {
          ...
          if (oldValue && v) {
            ...
            this.stack[y1 + offsetY] &&
              (this.stack[y1 + offsetY][x1 + offsetX] = oldValue + v);
          }
        }
      }),
    );
    ...
    this.score += Math.ceil(
      SCORE[lineCnt] * Math.pow(SCORE_BONUS, this.level - 1),
    );
